...
Object.defineProperty(Expander, 'perform', {
  value: function(In, mask, includeFiles, includeDirs) {
    ...
    var extras = [];
    var loadExtras = function(name, found) {

      // TODO m[1..n] match parts into ex
    ...
    var restore = function(found) {
      ...
      if (relative) {
        ...
        if (restored[0] == '.') {
          ...
          return loadExtras(restored, found);
        }
        return loadExtras('.' + path.sep /* windows? */ + restored, found);
      }
      return loadExtras(path.normalize(found.filename), found);
    }
    ...
    return promise(function(resolve, reject, notify) {
      ...
      Expander.recurse(
        ...
        function() {
          ...
          Object.defineProperty(results, 'ex', {
            value: extras
            // enumerable: true // cant show, fails tests
          })
      ...
      if (deeper > 0) { // **
        ...
        if (next) {
          if (next.indexOf('*') >= 0) {
            ...
            if (jump == 2) { // ?  /// no, this is broken, see finds the elephant
              jumping = next.split(path.sep);
              jumping.shift()
              next = jumping.join(path.sep);
            }
            ...
            sequence(files
              ...
              .map(function(name) {
                return function() {
                  ...
                  if (!stat.isDirectory()) {
                    if (deeper > 0) {
                      if (parts.length - depth - deeper - jump - 2 > 0) return; //?
                    } else {
                      if (parts.length - depth - jump - 2 > 0) return;
                    }
