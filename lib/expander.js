...
Expander.perform = function(In, mask, doFiles, doDirs) {
  return promise(function(resolve, reject, notify) {
    ...
    if (!(mask = Expander.invalid(mask, reject))) return;
    ...
    if (path.isAbsolute) absolute = path.isAbsolute(mask)
    else absolute = path.resolve(mask) == mask;
...
Expander.recurse = function(match, next, jump, Path, parts, depth, exinfo, found) {
  return promise(function(resolve, reject, notify) {
    ...
    match = parts[depth];
    while (depth < parts.length - 1 && !match.match(/\*/)) {
