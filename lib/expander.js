...
Expander.perform = function(In, mask, includeFiles, includeDirs) {
  ...
  return $$in.promise(function(resolve, reject, notify) {
    Expander.recurse(found, depth, 0, parts, Path, includeFiles, includeDirs)
    .then(
      function() {
        resolve(findings
...
Expander.recurse = function(found, depth, deeper, parts, Path) {
  return $$in.promise(function(resolve, reject, notify) {
...
Expander.match = function(Path, name, depth, jump, parts, stat) {
  ...
  try {
    ...
    // when starting at / there is no match in position
    regex = '^(.+)$';
  }
  // console.log({path: Path, name: name, match: match, regex: regex, prev: prev, parts:parts});
